	...
	<div class="panel panel-default" >
		...
		<table class="table-bordered table table-hover">
			...
			<tr >
				...
				@if($value->status)
					<td><span class="btn btn-danger" onclick="statu(this,{{$value->id}},1)" >禁用</span></td>
				@else
					<td><span class="btn btn-success" onclick="statu(this,{{$value->id}},0)" >正常</span></td>
				@endif

				<td><a href="javascript:;" onclick="edit({{$value->id}})" data-toggle="modal" data-target="#edit" class="glyphicon glyphicon-pencil"></a>
					&nbsp;&nbsp;&nbsp;<a href="javascript:;" onclick="deletes(this,{{$value->id}})" class="glyphicon glyphicon-trash"></a></td>
			</tr>
			@endforeach
		</table>
		<!-- 分页效果 -->
...
<script>
	...
	function statu(obj,id,status){
		...
		if(status){
			//请求地址,请求参数,响应结果
			$.post('/admin/admin/ajaxStatus',{id:id,"_token":"{{csrf_token()}}",
			"status":"0"},function (data){
				if(data == 1){
					$(obj).parent().html('<td><span class="btn btn-success" ' +
							'onclick="statu(this,'+id+',0)" >正常</span></td>');
				}else{
					alert('修改失败!');
				}
			})
			...
			$.post('/admin/admin/ajaxStatus',{id:id,"_token":"{{csrf_token()}}",
				...
				if(data == 1){
					$(obj).parent().html('<td ><span class="btn btn-danger" ' +
							'onclick="statu(this,'+id+',1)">禁用</span></td>');
				}else{
					alert('修改失败!');
				}
			})
